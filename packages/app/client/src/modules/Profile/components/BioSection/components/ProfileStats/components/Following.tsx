import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, ScrollArea } from "@spotify-social/components";
import { Search } from "@spotify-social/icons";
import { followingList } from '@/modules/Profile/data';
import { useState } from "react";


interface Props {
    numberOfFollowing: number;
    following: typeof followingList;
}

export default function Following({ numberOfFollowing, following }: Props) {
    const [search, setSearch] = useState('');

    const filteredFollowing = following.filter((follow) => {
        return follow.name.toLowerCase().includes(search.toLowerCase()) || follow.username.toLowerCase().includes(search.toLowerCase());
    });

    return (
        <Dialog>
            <DialogTrigger>{numberOfFollowing} following</DialogTrigger>
            <DialogContent>
                <DialogHeader>
